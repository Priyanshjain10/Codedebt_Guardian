...
def main():
    if not os.environ.get("GITHUB_TOKEN"):
        raise ValueError(
            "GITHUB_TOKEN environment variable is not set. This is required to interact with the GitHub API."
        )
...
def bad_function(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p):
    ...
    try:
        ...
        if pr:
            print(
                f"Success! PR URL: {pr.get('html_url', pr.get('url', 'Unknown URL'))}"
            )
        else:
            print(
                "Failed to create PR (returned None). Check branch logic and permissions."
            )
    except Exception:
        import traceback
