...
SAFE_AUTO_FIX_TYPES = {
    "bare_except", "hardcoded_password", "hardcoded_api_key", "unpinned_dependencies"
}
...
def _minimal_static_analysis(file_info: dict) -> List[dict]:
    ...
    cred_patterns = [
        (r'(password|passwd|pwd)\s*=\s*(?:"[^"]+"|\x27[^\x27]+\x27)', "hardcoded_password"),
        (r'(api_key|apikey|secret_key)\s*=\s*(?:"[^"]+"|\x27[^\x27]+\x27)', "hardcoded_api_key"),
    ]
    for i, line in enumerate(lines, 1):
...
def post_or_update_pr_comment(repo_full: str, pr_number: int, body: str, token: str):
    ...
    if existing_comment_id:
        update_url = f"{GITHUB_API}/repos/{repo_full}/issues/comments/{existing_comment_id}"
        update_resp = requests.patch(update_url, headers=headers, json={"body": final_body})
        update_resp.raise_for_status()
        logger.info(f"Updated existing PR comment {existing_comment_id} for {repo_full}#{pr_number}")
    else:
        post_resp = requests.post(comments_url, headers=headers, json={"body": final_body})
...
def process_pr_event(repo_full: str, pr_number: int, token: str):
    ...
    pr_resp = requests.get(f"{GITHUB_API}/repos/{repo_full}/pulls/{pr_number}", headers=headers)
    ...
    diff_resp = requests.get(f"{GITHUB_API}/repos/{repo_full}/pulls/{pr_number}", headers=diff_headers)
    ...
    logger.info(f"{repo_full}#{pr_number} Risk Score: {score} (CRIT:{critical_cnt} HIGH:{high_cnt} MED:{medium_cnt} LOW:{low_cnt})")
        ...
        try:
            ...
            if project:
                ...
                if existing:
                    ...
                    existing.summary = {
                        "critical": critical_cnt, "high": high_cnt,
                        "medium": medium_cnt, "low": low_cnt,
                ...
                db.commit()
                logger.info(f"PR scan record saved: {scan_id} for {repo_full}#{pr_number}")
            else:
        ...
        if should_fix:
            try:
                ...
                fix_proposal = {
                    ...
                    "before_code": "",
                    "after_code": "",
                    "steps": [f"Fix {issue['type']} at {issue.get('location', 'unknown')}"],
                }
