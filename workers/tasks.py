...
def run_scan_analysis(self, scan_id: str, repo_url: str, branch: str = "main"):
    ...
    try:
        ...
        _publish_progress(scan_id, "[1/4] 🕵️ Fetching repository data...", "detection", 8)
        ...
        try:
            ...
            if prev_scan and prev_scan.commit_sha and prev_scan.commit_sha != head_sha:
                ...
                if not changed_files:
                    has_changes = False
                    _publish_progress(scan_id, "[1/4] ⚡ Incremental Scan: No relevant files changed since last scan.", "detection", 15)
                else:
                    _publish_progress(scan_id, f"[1/4] ⚡ Incremental Scan: Found {len(changed_files)} changed files.", "detection", 15)
                    repo_data["files"] = (f for f in repo_data["files"] if f["name"] in changed_files)
        ...
        if has_changes:
            _publish_progress(scan_id, "[1/4] 🕵️ Running debt detection (Thread Pool)...", "detection", 20)
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                def scan_file(file_info):
                    try:
                        ...
                        # 2. AI (skip static for AI simplicity in batch)
                            ...
                            from agents.debt_detection_agent import JS_SUPPORT, _js_analyzer
                            if JS_SUPPORT:
                ...
                for i, future in enumerate(as_completed(futures), 1):
                    ...
                    if i % max(1, total_files // 10) == 0:
                        _publish_progress(scan_id, f"[1/4] 🕵️ Scanned {i}/{total_files} files...", "detection", 20 + int((i/total_files)*10))
        ...
        try:
            ...
            _publish_progress(scan_id, f"[1/4] ✅ Scanned {files_scanned} files — found {total_issues} issues", "detection", 30)

            # Phase 2: Priority Ranking
            _publish_progress(scan_id, "[2/4] 📊 Ranking issues by business impact...", "ranking", 35)
            ...
            _publish_progress(scan_id, f"[2/4] ✅ Ranked {len(ranked_results)} issues — {critical} CRITICAL, {high} HIGH", "ranking", 55)

            # Phase 3: Fix Proposals
            _publish_progress(scan_id, "[3/4] 🔧 Generating AI fix proposals...", "fixes", 60)
            fix_proposals = orchestrator.propose_fixes(
                ranked_results[:10],
                project_id=str(scan.project_id) if scan.project_id else None
            )
            _publish_progress(scan_id, f"[3/4] ✅ Generated {len(fix_proposals)} fix proposals", "fixes", 80)

            # Phase 4: TDR + Hotspots
            _publish_progress(scan_id, "[4/4] 🔥 Computing health score & hotspots...", "metrics", 85)
            hotspots = []
            tdr = {}
            try:
                from tools.hotspot_analyzer import HotspotAnalyzer
                hotspots = HotspotAnalyzer().analyze(detection_results.get("issues", []))
            ...
            _publish_progress(scan_id, f"[4/4] ✅ Analysis completed in {duration:.1f}s", "complete", 100)
            _publish_complete(scan_id, {
                "summary": summary,
                "tdr": tdr,
            ...
            logger.info(f"Scan {scan_id} completed: {total_issues} issues in {duration:.1f}s")
            ...
            if scan.project_id:
                ...
                if project:
                    ...
                    if team and scan.triggered_by:
                        log_action_sync(db, team.org_id, scan.triggered_by, "scan.completed", {
                            "scan_id": scan_id,
                            "total_issues": total_issues,
                            "grade": tdr.get("grade", "N/A"),
                        })
...
def generate_embeddings(scan_id: str, project_id: str, repo_url: str, branch: str = "main"):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            files_to_embed = []
            valid_exts = (".py", ".ts", ".tsx", ".js", ".jsx", ".go", ".java", ".cpp", ".c", ".h", ".md")
            ...
            if files_to_embed:
                ...
                logger.info(f"Finished embedding {len(files_to_embed)} files for project {project_id}")
            else:
                logger.warning(f"No embeddable files found in {repo_url}")
                
    except Exception as e:
        logger.error(f"Failed to generate embeddings for project {project_id}: {e}", exc_info=True)
...
def create_fix_pr(repo_url: str, fix_proposal: dict, issue: dict, scan_id: str = ""):
    ...
    try:
        ...
        if pr and scan_id:
            _publish_progress(scan_id, f"✅ PR created: {pr.get('title', '')}", "pr", 100)
        return pr
    except Exception as e:
