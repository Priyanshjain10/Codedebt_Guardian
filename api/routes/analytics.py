...
async def get_analytics_stats(
    ...
    scans_run = len(scans)
    total_issues = sum(s.summary.get("total_issues", 0) for s in completed_scans if s.summary)
    fixes_generated = sum(s.summary.get("fixes_proposed", 0) for s in completed_scans if s.summary)

    # TODO: Once PR creation populates PullRequest properly, this will be accurate.
    # We can also join against PullRequest.
    pr_query = select(func.count(PullRequest.id)).join(Scan).where(
        and_(
            Scan.triggered_by == user.id,
    ...
    debt_trend = []
    category_breakdown = {
        "security": 0, "performance": 0, "maintainability": 0,
        "complexity": 0, "documentation": 0, "testing": 0, "dependencies": 0
    }
    severity_breakdown = {
