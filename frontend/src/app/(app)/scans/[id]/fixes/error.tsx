...
export default function ErrorBoundary({
    ...
    useEffect(() => {
        ...
        console.error('Fixes Load Error:', error);
    }, [error]);
        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] p-6">
            <div className="bg-bg-card border border-border rounded-xl p-8 max-w-md w-full text-center space-y-6 shadow-sm">
                ...
                <div className="space-y-2">
                    <h2 className="text-lg font-semibold text-text-1">Failed to Load Fixes</h2>
                    <p className="text-sm text-text-2">{error.message || 'There was a problem preparing the AI fix suggestions.'}</p>
                </div>
