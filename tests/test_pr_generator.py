...
class TestPRGenerator:
    def setup_method(self):
        self.env_patcher = patch.dict(
            "os.environ", {"GITHUB_TOKEN": "fake-token", "GOOGLE_API_KEY": "fake-key"}
        )
        self.user_patcher = patch(
            "tools.pr_generator.PRGenerator._get_username", return_value="test-user"
        )
    ...
    @patch(
        "tools.pr_generator.PRGenerator._get_file",
        return_value=("mock content", "mock-sha"),
    )
    ...
    def test_pr_logic(
        ...
        mock_response.json.return_value = {
            "number": 1,
            "title": "fix(Bug): [CodeDebt Guardian] Fix bugs",
            "html_url": "url",
            "state": "open",
        }
...
class TestApplyFix:
    ...
    def test_string_replace_succeeds(self):
        ...
        fix_proposal = {"before_code": "return 1", "after_code": "return 2"}
        issue = {"type": "bug"}
